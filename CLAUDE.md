...
| D37 | A subscribe the server **refuses out loud** reports what it said, not what a shortfall infers. `OnStatus` records the message of any non-`success` status against whichever acknowledgement slot is armed, and `MassiveStreamSubscriptionException` carries it as `ServerMessage` — `null` when the server said nothing. The wait is **not** short-circuited, and no second exception type is introduced. | D33 made acknowledgement counting the mechanism because the server **says nothing** about a topic it does not recognise, so a shortfall is the only evidence there is. That holds for silence and not for a refusal: `{"ev":"status","status":"error","message":"not authorized"}` answered `NOI.*` on an unentitled key (2026-09-08), the SDK discarded it, and the caller was told the server ignores a topic code it does not recognise — sending them to check their spelling when the fix is their plan. Reachable today on `T` or `Q` for any Basic stocks key, so it is not the new topics' problem. The window still runs in full because that same frame arrived **ahead** of the acknowledgements for the pairs that were accepted: completing early would report still-in-flight pairs as unacknowledged, trading a wrong message for wrong data. A refusal names no pair, so it cannot be attributed by content the way an acknowledgement is; it goes to whoever is **sending**, which is the inverse of D33's crediting order and is why the two read as contradictory until you notice one has text to match and the other does not. Two slots, cleared on arming as well as teardown, for D33's own reason — a caller subscribing inside a replay's verification window must neither erase the replay's reason nor inherit it. A distinct exception type was rejected: the only thing a separate catch clause buys is knowing the failure is **permanent**, and `error` covers unrelated causes distinguished only by prose, exactly as `auth_failed` does under D35 — a category invented from one observation is the second source of truth D18 forbids, arriving on the exception surface. Scrubbing the prose for rule 11 was also rejected: D35 already echoes `auth_failed` verbatim at the one point the key genuinely was just sent, so scrubbing only here is inconsistent, and the exact check reaches for `ApiKey` to build an exception where D35 deliberately wipes it from a rented buffer. |
...
| D38 | The fractional-share family -- `dv`, `dav`, `ds`, `decimal_size`, `decimal_volume`, 31 sites the description declares `type: string` -- binds to **`decimal`**, on both packages, named on the map row like any other type override. `JsonValueReader.ReadDecimal` **proves** each parse lossless by formatting the result back and comparing bytes, refusing anything that does not match; `JsonValueWriter.WriteDecimalString` returns it to the wire as a string. Prices stay `double`. | Every row of a trades page allocated a string for a number: `Trade.DecimalSize` is `required`, so every row paid for one, and a `decimal` lives in the row instead. Measured through the public API on 2026-09-09, a 50,000-row trades page went from 9,122,432 B to 7,922,432 B -- 1,200,000 B and 50,000 GC-tracked objects, net of the 400,000 B the array itself gains, since `Trade` grows from 112 to 120 bytes when an 8-byte reference becomes a 16-byte value. That is D32's argument, and the piece D32 left on the table, since its closed set had no `decimal` for the generator to bind. The wire's own distinction is followed rather than second-guessed -- these five fields are declared strings precisely because they cannot survive as JSON numbers, and prices are declared numbers -- so `double` is not on the table here and `decimal` is not on the table there. Round-tripping rather than digit-counting is the load-bearing part: `decimal.TryParse` **rounds silently and returns `true`** when a value's scaled integer overflows its 96-bit mantissa, which digit count does not predict -- `8827.8140001491929689677164477` rounds at 29 digits while `12345678901234567890123456789` does not, and a threshold set at 29 leaked 22,300 silent roundings across a 3,000,000-value corpus, where the round-trip check leaked none in either direction. It costs 19 ns against a parse that already costs 54, and it is what keeps this from being the silently-wrong binding D16 and D32 exist to refuse. The guard enforces canonical spelling as a side effect -- `.5`, `+4989.0`, `004989.0`, `4989.`, `1e5` are all refused -- which is stricter than "never rounds" and is accepted deliberately: Massive's wire is canonical today -- checked against the live service on 2026-09-09, across a fifty-trade page and the snapshot sites, which is the one claim here a fixture structurally cannot make and so is pinned by a live test -- a refusal is visible where a normalisation is not, and if a non-canonical form is ever observed the fix is a normalising pre-pass, not abandoning the guard. Every official client binds this family to a string, which was checked and is close to neutral: none of them was choosing between a string and a decimal, because Go, JS, and Python have no such type to reach for, and the Go client's stated reason -- "kept as a string so that the precision the server sends is preserved" -- is a description of `decimal`, not an argument against it. Two costs are taken knowingly. A value type carries no presence check, so `Trade.decimal_size` no longer throws when absent; that check was never backed by anything trustworthy, since the description's requiredness is unreliable (D17) and this model's own siblings are typed nullable in the map because the service omits fields it marks required -- the check existed only because a required reference type needs `required` to satisfy CS8618. And nine public properties change type, which is free before #19 tags anything and a changelog-noted break the moment it does. |
| D39 | A malformed **field** drops one event, counted on its own `MalformedCount` and reported through its own throttled `MalformedObserved` carrying the `JsonException`; a malformed **frame** stays terminal. `JsonValueReader`'s range failure echoes the offending token, capped at 32 bytes; its two sibling messages do not. | One `z` the reader refused ended a connection that multiplexes trades, quotes and both aggregate windows — four times in ten minutes in production — so one symbol's off-schema field took down the trade feed sharing that same socket. The catch sits in `TopicSink<T>.Write` because `Dispatch` hands it a struct **copy** of the frame reader while the real one is already parked on the failed object's end token: the loop resumes with no resync, and the sink is also the type that knows `T` and owns the count. The boundary is drawn there and nowhere wider — a malformed `ev` throws from `ReadEventCode`, which drives the real reader, stranding it mid-object with no topic to attribute the loss to, so that stays terminal. G3's reasoning is untouched and strengthened: the SDK now neither reconnects **nor** stops over a parse failure; only its conclusion that the failure is terminal changed. A second counter and a second signal were chosen against D33's "a second signal is strictly less surface for the same information", because it is **not** the same information — a slow consumer raises `TopicBufferCapacity` and an off-schema vendor cannot do anything, so one counter would hand a consumer advice that cannot work, which is #64's complaint reproduced deliberately; each throttle keeps its own window so a burst of one cannot mask the other. The exception travels with the raise because the two halves of #65 work against each other: the improved message reached consumers through `Faulted` precisely **because** the failure was terminal, so making it non-terminal without carrying the exception would swallow the evidence and leave the question of what `z` should be permanently undecidable. The echo reverses `JsonValueReader`'s standing "no method echoes the offending value" contract, and the confinement is the rule 11 argument rather than care taken per site: `OutOfRange` is reachable only after a caller has checked `TokenType == Number`, so its bytes are provably a JSON number and an API key is not one — where `NotExact` reads a String token, whose class of value is unbounded, and `Expected` names a token type and has nothing to quote. `StockAggregate.AverageTradeSize` stays `long`: Massive documents `z` as an `integer`, so retyping it would be a breaking public change made on a guess contradicting the vendor, and this work is what produces the evidence to revisit it. |
| D40 | Teardown sends a close frame and does not wait for the reply: `IMassiveWebSocket.CloseOutputAsync` replaces `CloseAsync`, called from `DisposeAsync` and from the reconnect handover through one swallow-everything helper bounded by a private 2-second `Duration`. No public member is added. | `ClientWebSocket.Dispose()` **aborts** -- it does not perform the closing handshake -- so the SDK never told the server a disconnect was deliberate. That matters more than hygiene on a plan allowing one connection per cluster, where a client that aborts and immediately reconnects may collide with a connection the server has not yet reaped. The handshake is not merely skipped but structurally **unreachable** on both teardown paths: `CloseAsync` waits for the server's close reply, and collecting it needs something pumping `ReceiveAsync` -- `DisposeAsync` has already cancelled `_shutdown` and joined `ReadLoopTask`, and the reconnect handover runs ON the read loop. So the literal fix issue #66 proposed would park every teardown until a timeout for a reply that cannot arrive. Closing earlier instead is worse: the loop would receive our own close frame, which `FrameReader` reports as `MassiveStreamException` and G3 treats as reconnectable, so a polite close would trigger a reconnect on the way down. `CloseOutputAsync` **replaces** `CloseAsync` rather than joining it, because an added member would leave the declared-implemented-uncalled shape #66 exists to remove, merely relocated; keeping the old name over send-only semantics was rejected as a name that misleads. The reconnect call site is not decorative: the adapter's `State is Open or CloseReceived` guard has two live arms -- a message past `MaxMessageBytes` faults the loop with the socket still `Open`, and a server-sent close leaves it `CloseReceived`, which the SDK now answers for the first time. A genuine abort leaves it `Aborted` and is skipped, which is the case the production logs actually show, so this makes no claim about those. The timeout is a private constant, not a `MassiveStreamOptions` property, because it is public surface under rules 10 and 14 for a number no consumer can tune; `HandshakeTimeout` is not reused because "handshake" here means the opening one and its 10s is sized for a different quantity. The token source is standalone rather than linked to `_shutdown`, which is load-bearing: `DisposeAsync` cancels `_shutdown` before reaching the socket, so linking would silently stop the frame ever being sent while every "did not throw" test still passed. Every failure is swallowed, matching `DisposeAsync`'s two existing catch blocks, and the description is always `null`, which puts rule 11 out of reach by construction. The benefit rests on one more assumption this row names because no offline test can verify it: `ClientWebSocketAdapter.DisposeAsync` aborts the socket immediately after `CloseOutputAsync` returns, which is safe only because `ClientWebSocket.Dispose()` closes the underlying socket with default linger, so bytes already handed to the kernel still drain. `MassiveStockStream` gains no public shutdown member: `await using` already gets the close, a second teardown member would be more surface for the same thing, and an additive member can be added later without a break. The full RFC 6455 handshake is deferred, not refused -- it needs a pump alive during teardown, which means reordering disposal and teaching the read loop to recognise its own deliberate close so G3 does not reconnect through it. |
| D41 | Both date converters check the raw token length **before** the unescaped fast path, not inside the copying one, so `Parse` is unreachable with more than `MaxRawLength` (64) bytes and every value its failure messages quote is bounded. Nothing is lifted out of `JsonValueReader`, and no message stops naming the value it refused. | `LocalDateJsonConverter` and `InstantJsonConverter` already refused a value past 64 bytes -- but only on the copying path, which runs for an escaped value or one split across buffer segments. The fast path, which is every response the service actually sends, called `Parse(reader.ValueSpan)` with **no length check at all**, so a pathological String token was rendered into the exception message whole and, through the DI package's log bridge, into a log line. That is the unbounded echo D39 capped for numbers, arriving by a path D39 did not look at. #67 proposed lifting `EchoNumber` into a shared helper and deciding once whether a String echo is capped or refused; reading the code retired that fork. One guard per converter fixes all four echo sites at once, and the bound it establishes is structural in exactly the way `OutOfRange`'s is -- `Parse` is `private static` with both call sites past the check, so a third caller cannot be added without confronting the invariant, which is #67's own complaint that the safety here is circumstantial, answered. Capping to 32 was rejected as strictly worse: it buys a smaller number at the cost of reintroducing the multi-byte-truncation caveat `EchoNumber`'s remarks carry, which cannot arise when a value is echoed whole or not at all, and it does not settle the rule 11 question either, since a 32-byte String can be an API key exactly as a 64-byte one can. Refusing to echo was rejected because `found '2024-13-45'` is the entire diagnostic value these messages have, and refusing buys nothing real: a key that ever reached a `format: date` field would already be sitting in the caller's deserialized object graph on the success path, so declining to print it defends nothing. 64 is not an arbitrary cap but the constant already meaning "longer than any escaped spelling of a valid value", so anything echoed stays inside a plausible-value envelope. The accepted cost is that a malformed date longer than 64 bytes now reports `found a longer value` rather than naming itself -- which is what the copying path has always done, so it is not a new class of message -- plus one length read on the fast path, which allocates nothing. |
| D42 | A `max_connections` status is recorded for the **whole connection**, not only for whatever request happens to be in flight. `StopPermanently` substitutes `MassiveStreamEvictedException` for the drop the status precedes, and the eviction is separately kept as `EvictionCount` and `LastEvictionMessage`, cumulative, beside `ReconnectCount` and `LastReconnected`. The pending cause is scoped to one socket; the counters are never cleared. The status **also** stays the refusal of a subscribe in flight. Not terminal -- the reconnect policy is unchanged -- and no new event. | Eviction is the only documented disconnect cause that announces itself, and it was the only one the SDK could not report. D37 kept a refusal only while `_pendingAcks` or `_replayAcks` was armed, which is right for a subscribe and leaves a status aimed at the connection no path at all: with nothing in flight the message was discarded and the consumer saw a bare abort carrying no close code, indistinguishable from a slow consumer or a network drop -- so an operator audits their own throughput while the real cause is a second process on the same key. Massive's knowledge base documents the message and says the OLDER connection is the one closed on `/stocks`, `/futures`, `/indices` and `/forex`; this repo saw it live on 2026-09-09, where it reached a test only because a subscribe happened to be in flight. Nothing is raised from `OnStatus`: it runs on the read loop and the paths around it hold `_ackLock`, which is D33's deadlock shape. It is not terminal because D35's argument does not transfer -- a refused key is refused on every retry, while an eviction is caused by another process and need not recur -- and the cost is named rather than avoided: two clients on a one-connection plan will keep displacing each other, and the remedy (another connection on the plan, or one fewer process) is not the SDK's to choose. It is the connection's cause **and** the subscribe's refusal because both are true and answer different questions; routing it to one would either regress the 2026-09-09 observation or leave the connection unexplained, and recording it consumes nothing the acknowledgement bookkeeping needs. The cumulative pair is the half a `Faulted`-only fix would have missed, and it is not optional: reconnect is **on by default**, `TryReconnectAsync` returns false only when it is `null`, and every other path either reconnects or exits through the cancellation filter -- neither of which raises `Faulted` -- so reporting only through the exception would do nothing on the default configuration, which is precisely the two-clients case that motivated the work. A second event was still rejected under D33: a property carries the same distinction D39 needed a second signal for, at no event cost, because `Reconnected` already fires at the exact moment the question is asked, so an `Evicted` event would be a new signal firing at the same instant as an existing one. The pending cause is socket-scoped because the status precedes the close it explains -- left armed for the connection's life it would charge an unrelated failure hours later to an eviction already survived -- while the counters are never cleared because "has this stream ever been evicted" is a different question from "was this drop one". `ReadLoopTask` rethrows the substituted exception rather than the raw drop, so it and `Faulted` cannot disagree about why the stream stopped, which is the shape G5 already gives an authentication failure surfacing during a reconnect. Rule 11 holds by the same structure as D35: the text is the server's own prose, and the key travels only in the SDK's auth frame, never in a reply. One thing stays unobserved and this row says so rather than implying otherwise -- whether the read loop actually reads the status before the abort on an established connection with nothing in flight. The vendor says it is sent; a slow consumer may never read a frame sent just before a drop. If it never arrives, the cost is a path that does not fire and counters that stay at zero, and nothing else about the stream changes. |
| D43 | The throttle's limiter is built with `AutoReplenishment = false`, and the handler owns the timer that calls `TryReplenish()`. `ReplenishmentPeriod` stays `Window / PermitsPerWindow` at **tick** precision; the handler's tick interval is `clamp(period / 2, 5 ms, 1 s)` and sets only how late a permit can be, never the rate. No public member is added. | The automatic path adds a flat `TokensPerPeriod` on every firing and, in the runtime's own words, trusts the timer to be close enough -- so the delivered rate was whatever `System.Threading.Timer` could express, and three unrelated-looking failures had one cause. A period is given to the timer in whole milliseconds, so a fractional one over-delivered: 2.5 ms became 2 ms, measured at 497 a second against a configured 400, and #70 measured +7% at 7,000 a minute and +15% at 45,000. A period under 1 ms truncates to 0, which `Timer` stores as fire-once, so the limiter refilled once at construction while the bucket was still full and never again -- measured as 1 request released in two seconds against a configured 1,500, with `QueueLimit`'s `int.MaxValue` default meaning everything else waited forever and nothing reported it. And a firing the runtime delayed lost its permit for good, which is what made exactly 1 ms under-deliver. The manual path instead adds the fill rate times the time that actually elapsed, with the rate computed from `ReplenishmentPeriod.Ticks`, so none of the three is reachable: a fractional period is honoured exactly, a sub-millisecond period works, and a late tick catches up. That was true and incomplete -- the manual path introduced a fourth failure of its own, which shipped in 0.4.0 and is the last paragraph of this row (#72). Measured across #70's own table after the change, every row from 600 to 120,000 a minute delivers 99-100% of what was configured. Keeping automatic replenishment and choosing whole-millisecond periods with a larger `TokensPerPeriod` was rejected: it is exact only by making the refill a block instead of a slice -- 7,000 a minute divides into whole milliseconds only as seven tokens every 60 ms, which is precisely the block refill this handler's period exists to avoid, since two adjacent blocks can land in one server window -- and any period small enough to stay a slice is fractional again, so it reintroduces the same rounding by another route. The tick interval is a latency bound rather than a rate, and the clamp is what makes the bound a fact instead of luck: the limiter ignores a replenish arriving before a full period has elapsed and dates the next from when it accepted rather than when it was due, so a tick scheduled one period after the last lands a hair short from time to time and is skipped, and the permit waits one interval. On the free tier's 12 s period, permits 6/7/8 arrived at 12.0/24.0/37.0 s against an ideal 12/24/36 -- one interval late, once -- where removing the cap gave 42.0 s; an interval equal to the period measured 36.0 s in the same run, which is drift falling favourably rather than structure, and is why this is stated as a bound. The 5 ms floor is what keeps the ticker off the fire-once cliff the fix exists to remove, costing 200 wakeups a second while idle against the thousand-plus a second the caller configured; the 1 s cap costs one wakeup a second on any allowance slower than two a second and buys a latency bound that no slower allowance would otherwise have. Both are private constants rather than `MassiveRateLimitOptions` properties for D40's reason. The gates are **one-sided**, which is what lets a wall-clock assertion sit in the offline tier against D31's "time is not gated": the stall test is all-or-nothing, and the over-delivery test asserts only an upper bound on delivered rate, which load and jitter can lower but never raise -- neither is the two-sided tolerance D31 keeps out, so a shared runner cannot flake them. The accepted cost is that they prove **red** only on a machine quick enough to reach the broken code's own ceiling, about 500 timer firings a second for the over-delivery row; on a much slower one the unfixed tree would under-deliver and the test would pass for the wrong reason, so the red proofs are recorded here and were run on the development machine, never in CI. The tick interval itself is argued and measured but deliberately **not** gated: a latency assertion is an upper bound on time, which is the one direction a loaded runner genuinely can fail. The 1-tick floor on the period survives, now reachable only above roughly 600 million permits a minute; refusing it in `Validate` was rejected as a public behaviour change for a configuration nobody holds, and at ten million requests a second the allowance is not the constraint. Disposal order is load-bearing only in the cheap direction: the ticker goes first, and a firing already under way was harmless while the callback was a bare `TryReplenish`, which returns without touching a disposed limiter -- checked rather than assumed, since an unhandled exception raised on a thread-pool timer callback ends the process. **The fourth failure, #72.** Driving replenishment by hand moved the clock into the handler's hands without moving all of it: the limiter returns from `TryReplenish` while the bucket is full **before** dating itself, and acquiring a lease never dates it either, so its clock freezes for exactly as long as a caller is idle. The first tick after a request finally drains the bucket then adds the fill rate times that whole idle, capped at `TokenLimit` -- the burst, and then the entire allowance again immediately behind it. Measured on the free tier, 65 s idle released **10 requests inside one second** against a configured five a minute, where 0.3.0's automatic path released 5: a flat `TokensPerPeriod` per firing has the right idle behaviour and the wrong rate, and the manual path has the right rate and the wrong idle behaviour. Every gate written for #70 spends the burst before it measures anything, which is exactly why this sat underneath a table reading 99-100% on every row -- the bucket was never left full and stale, so the idle path went entirely unexercised. The handler now keeps its own record of when the two clocks were last in step and, once a full period's credit has banked with the bucket still full, takes one permit and replenishes: that gives the limiter the room it needs to accept the elapsed time and date itself from it, discarding credit a full bucket could never have held anyway. Nudging on **every** tick, which is what the issue proposed, stops the second burst the same way but spends a permit whether or not the credit to replace it has accrued, so a slow allowance sits permanently one short -- measured 39 of 40 at the gate, and 4 of 5 on the free tier, a 20% haircut on the burst D30 calls the difference between an SDK that works on that tier and one that 429s on the sixth call. The staleness guard is what makes the nudge net-zero instead, because it fires only where the replenish is guaranteed to return the permit it took. Owning the bucket outright was never on the table: that reverses D30, and queueing, fairness and cancellation are the parts D30 declined to own. After the fix the same probe releases 5 after any idle, and permits 6/7/8 land at 12.00/24.00/36.00 s -- burst intact, cadence exact, and better than the 12.0/24.0/37.0 recorded above. Two costs are named rather than avoided. The nudge leans on the same undocumented behaviour the rest of this row already leans on -- that a replenish into a bucket with room adds the fill rate times the elapsed time and dates the limiter from now -- so it adds no new dependency on runtime internals; but if a future runtime ever dates the clock on a full bucket too, the nudge becomes unnecessary and starts costing a permit once a period rather than saving one, and `KeepsTheWholeBurstAvailableAfterSittingIdle` asserts exactly that quantity, so it is what would report it. And the disposal race called harmless above no longer is, because `Tick` now reads statistics and takes a permit: it catches `ObjectDisposedException` itself, and an `Interlocked` guard keeps two overlapping firings from spending two permits against credit that guarantees only one. |
| D44 | The retry handler also retries a **transport failure**: an `HttpRequestException` thrown in place of a response, whose `HttpRequestError` is on an **allow-list** of the categories that describe the connection (`Unknown`, `NameResolutionError`, `ConnectionError`, `SecureConnectionError`, `HttpProtocolError`, `ResponseEnded`). It uses the same `MaxAttempts` and backoff, and never retries once the handler's token is cancelled. A timeout and a failure while reading the body stay unretried. | The handler's remarks promised this from `5c0fd04`, but the loop retried on status alone, so a connection reset failed the call on its first attempt however high `MaxAttempts` was set (#73). `Unknown` is on the list because it is what the runtime actually reports for a reset mid-send: `HttpConnection.MapSendException` wraps a bare `IOException` under it (dotnet/runtime v10.0.0). A list of only the categories that look like connection failures would have missed the one case observed. The same category also carries the runtime's wrap of an `InvalidOperationException` mid-send, which a retry will not cure; that costs at most `MaxAttempts` sends too. It is an allow-list rather than a deny-list so that a category a later runtime adds keeps today's behaviour until someone decides otherwise. The six left out fail identically on every attempt, the transport-level equivalent of a non-429 4xx: they describe an answer (`InvalidResponse`, `ConfigurationLimitExceeded`), a negotiation (`VersionNegotiationError`, `ExtendedConnectNotSupported`), or credentials and proxies (`UserAuthenticationError`, `ProxyTunnelError`). `SecureConnectionError` is retried knowingly. A handshake reset is transient and a certificate refusal is not, the category cannot tell them apart, and guessing wrong costs at most `MaxAttempts` sends. The cancellation check is not decorative. `SocketsHttpHandler` already reports a failure under a cancelled token as a cancellation, but an inner handler need not, and with a zero backoff nothing else stops a second send for a caller who has given up. The same check leaves an `HttpClient.Timeout` unretried, since that timeout cancels this token. Neither a timeout nor a body-read failure could be retried here anyway, and the remarks now say so: `HttpClient.Timeout` spans the whole pipeline, and under `ResponseHeadersRead` the body is read after this handler has returned. |
...
| `Task.Delay` | produce | `Task.Delay(backoff.ToTimeSpan(), cancellationToken)` | `MassiveRetryHandler.WaitAsync` |
...
| `new Timer(…, TimeSpan, TimeSpan)` | produce | `interval.ToTimeSpan()`, twice | `MassiveRateLimitHandler` ctor |
...
| `CancellationTokenSource.CancelAfter` | produce | `expiry.CancelAfter(window.ToTimeSpan())` | `RateLimitHandlerTests` |
| `Task.Delay` | produce | `Task.Delay(idle.ToTimeSpan(), Ct)` | `RateLimitHandlerTests` |
...
- **Allocation**: build request URIs through `RequestUriBuilder`, never `UriBuilder` or a
  ...
  that converter's closed type set -- `string`, `bool`, `int`, `long`, `double`, `decimal`, their
  nullable forms, and arrays of those -- fails generation rather than binding to something that
  throws at deserialization. Extending the set means adding a reader to `JsonValueReader` first.
  `decimal` is the one member whose wire token is not its own: it reads a JSON string and writes
  one back through `JsonValueWriter`, and the map names it on the row of a field the description
  declares `type: string` but documents as a number (D38).
