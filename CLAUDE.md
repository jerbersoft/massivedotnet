...
| D33 | Streaming topics are a typed enum, and every subscription a **caller** makes is acknowledgement-counted; a shortfall throws. A **reconnect's replay is not**: it re-sends the registry and reports success without waiting for acknowledgements. | The server acknowledges a valid topic with an unknown ticker and **silently drops an unknown topic** — no acknowledgement, no error (probed 2026-09-07). So the usual posture of letting the server reject a bad value (D9) does not apply: it does not reject, it ignores, and a caller would see a healthy connection producing nothing forever. The enum makes the observed mistake unrepresentable; counting guards the ones not yet observed, because a subscription that silently does not exist is indistinguishable from a quiet market (D29's argument). The replay is the acknowledged gap, found by the whole-branch review rather than designed: the replay runs **on the read loop**, and the acknowledgements it would wait for are delivered **by that same loop**, so it cannot await them without deadlocking itself. The consequence is exactly the failure this row's first half exists to prevent, reached by another road — a reconnect the SDK reports as successful can leave a topic silently unsubscribed, and `Reconnected` and `ReconnectCount` say it worked. Closing it needs deferred verification the loop checks as acknowledgements arrive, with a deadline and a signal on shortfall; that is new machinery, so it is recorded here rather than half-built. |
...
src/MassiveDotNet.WebSocket WebSocket streaming client: reconnect with backoff, per-topic
                            backpressure, and the hand-written event converters (D33-D35).
...
| `Task.Delay` | produce | `Task.Delay(Duration.FromMilliseconds(20).ToTimeSpan(), ct)` | `DispatchTests` |
